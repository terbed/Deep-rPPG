...
def eval_rateprob_results(ref, ests: tuple, sigs: tuple, labels: tuple):
    """
...
def eval_rate_results(ref, ests: tuple, sigs: tuple, labels: tuple):
    ...
    for i, est in enumerate(ests):
        ...
        plt.title(labels[i])
        plt.plot(t, ref, 'k', linewidth=2, label='reference')
        plt.plot(t, est, 'r--', linewidth=1.5, label='estimate')
        plt.legend()
        plt.grid()
    plt.show()
...
if __name__ == '__main__':
    ...
    #
    eval_rate_results(refs, ests=(rates_rate, ), sigs=(signal_rate, ),
                          labels=('CNN+LSTM',))